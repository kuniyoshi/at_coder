...
impl ReadInput {
        ...
        U::Err: Debug + 'static,
    {
        ...
        let a: T = parts
            .next()
            .ok_or(Box::new("Missing first element") as Box<dyn Debug>)?
            .parse()
            .map_err(|e| Box::new(e) as Box<dyn Debug>)?;
        let b: U = parts
            .next()
            .ok_or(Box::new("Missing second element") as Box<dyn Debug>)?
            .parse()
            .map_err(|e| Box::new(e) as Box<dyn Debug>)?;
        Ok((a, b))
    }
        ...
        V::Err: Debug + 'static,
    {
        ...
        let c: V = parts
            .next()
            .ok_or(Box::new("Missing third element") as Box<dyn Debug>)?
            .parse()
            .map_err(|e| Box::new(e) as Box<dyn Debug>)?;
        Ok((a, b, c))
    }
                    ...
                    Ok(mut parts) => {
                        if parts.len() == 2 {
                            ...
                            return Err(Box::new(format!(
                                "Error: Line {} does not contain exactly two elements.",
                                i + 1
                            )) as Box<dyn Debug>);
                        }
                    }
        ...
        T::Err: Debug + 'static,
    {
        ...
        if let Some(Ok(line)) = lines.next() {
            let parsed_values: Result<Vec<T>, _> =
                ...
                match row {
                    Ok(row) => {
                        if row.len() == w {
                            ...
                            return Err(
                                Box::new("Row length does not match width") as Box<dyn Debug>
                            );
                        }
                    }
